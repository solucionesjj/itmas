...
H · Sistema de diseño — I · Estadísticas del producto SAC
...
## I. Estadísticas del producto SAC

> **SAC** es el sistema de administración de cobranzas que Effective Computer Solutions ofrece a
> entidades financieras para la recuperación de cartera. Cada cliente tiene su propia nube instalada,
> con uno o varios motores de base de datos. Esos motores generan **diariamente** un juego de
> estadísticas por base de datos (tamaño, deudores, cuentas, sumatorias financieras, gestiones y
> usuarios activos) que hoy vive solo en el motor de origen. El área de Infraestructura necesita
> centralizarlas en IT-MAS para monitorear el parque completo de bases de datos y analizar su
> crecimiento.
>
> Este tema es un **catálogo independiente**, igual que el de EXT-1: no se cruza con el modelo
> `devices`/`inventories` de los agentes de recolección, aunque **reutiliza** el mecanismo de
> autenticación por clave de API de nodo para la ingesta. Requiere un ADR nuevo (el siguiente
> disponible es `ADR-0018`) por tratarse de un dominio y unos endpoints fuera del contrato original
> de `spec.md` §12 — mismo tratamiento que ADR-0013 (EXT-1) y ADR-0016 (aprovisionamiento desde el
> portal).
...
`POST /access-events`) modela inventario de hardware/software y eventos de acceso, no métricas de
producto. Este elemento crea la colección y el endpoint de ingesta; la consulta (BL-032) y el
análisis (BL-033) se apoyan en él.

**Modelo de datos — colección `sac_statistics`**

El contrato de la API normaliza los nombres a inglés camelCase, como el resto de `/api/v1`. Mapeo
autoritativo contra las columnas del origen (SQL Server):

| Columna de origen | Campo del contrato | Tipo en Mongo | Significado |
|---|---|---|---|
| `[Base] nvarchar(128)` | `databaseName` | `String` (máx. 128) | Nombre de la base de datos |
| `[tamanoTotalBaseDatos] int` | `totalSizeGb` | `Number` | Tamaño total de la base (GB) |
| `[deudor] int` | `debtors` | `Number` | Cantidad de deudores |
| `[deudorCuentasActivas] int` | `debtorActiveAccounts` | `Number` | Cuentas activas por deudor |
| `[cuentasActivas] int` | `activeAccounts` | `Number` | Cantidad de cuentas activas |
| `[SumatoriaSaldo] numeric(18,2)` | `balanceSum` | `Decimal128` | Sumatoria de saldo |
| `[SumatoriaMora] numeric(18,2)` | `overdueSum` | `Decimal128` | Sumatoria de mora |
| `[SumatoriaCapital] numeric(18,2)` | `principalSum` | `Decimal128` | Sumatoria de capital |
| `[promedioUsuariosActivosTresUltimosMeses] int` | `avgActiveUsersLast3Months` | `Number` | Promedio de usuarios activos en los últimos 3 meses |
| `[gestiones] int` | `activities` | `Number` | Gestiones históricas (contador acumulado) |
| `[gestionesUltimos30Dias] int` | `activitiesLast30Days` | `Number` | Gestiones de los últimos 30 días (ventana móvil) |
| `[usuariosActivos] int` | `activeUsers` | `Number` | Usuarios activos |
| `[tamanoLog] int` | `logSizeGb` | `Number` | Tamaño del archivo de log (GB) |
| `[tamanoData] int` | `dataSizeGb` | `Number` | Tamaño del archivo de datos (GB) |
| `[fechaGeneracion] datetime` | `generatedAt` | `Date` (UTC) | Fecha de generación del registro |

Añade dos campos que no vienen del origen: `deviceId`, resuelto desde la clave de API del nodo que
reporta (trazabilidad de quién envió qué, sin una segunda consulta), y `_id`.
...
El motor de BD que reporta debe estar aprovisionado previamente como `devices` de categoría
`infrastructure` (`npm run device:provision` o `POST /devices`), exactamente como cualquier otro nodo.
No se añade una categoría `database`: cambiar la enum arrastraría al motor de alertas, que ya
...
Cada serie se agrupa **por nombre de base de datos**. No se deriva ningún "cliente" del patrón
`DBSAC_<NombreCliente>`: es una decisión explícita para no depender de una convención de nombrado en
las agregaciones. Un cliente con varias bases aparece como varias filas.

Indicadores requeridos y la métrica que los alimenta:

| # | Indicador | Métrica |
|---|---|---|
| 1 | Bases ordenadas de mayor a menor | `totalSizeGb` (por defecto) |
| 2 | Crecimiento mes a mes del tamaño de la base | `totalSizeGb` |
| 3 | Crecimiento mes a mes de las obligaciones | `activeAccounts` |
| 4 | Crecimiento mes a mes de los clientes/deudores | `debtors` |
| 5 | Crecimiento mes a mes de las gestiones | `activities` |
| 6 | Crecimiento mes a mes de los usuarios activos de los últimos tres meses | `avgActiveUsersLast3Months` |
| 7 | Crecimiento mes a mes de los datos financieros | `balanceSum`, `overdueSum`, `principalSum` |
